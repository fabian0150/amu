<?php
	...
	if (empty($email)) { 
		$row_array['code'] =  2;
		$row_array['error'] =  "E-Mail nicht angegeben";
		array_push($return_arr, $row_array);
	...
	if ($password_1 != $password_2) {
		$row_array['code'] =  2;
		$row_array['error'] =  "Passwörter stimmen nicht überein";
		array_push($return_arr, $row_array);
	...
	if (count($row_array) == 0) {
		...
		if ($result->num_rows > 0) {
			$row_array['code'] =  5;
			$row_array['message'] =  "User nicht registriert";
			$row_array['error'] = "Username bereits vergeben";
			array_push($return_arr, $row_array);
		...
		if ($result->num_rows > 0) {
			$row_array['code'] =  5;
			$row_array['message'] =  "User nicht registriert";
			$row_array['error'] = "E-Mail bereits vergeben";
			array_push($return_arr, $row_array);
			echo json_encode($return_arr);
			exit();
		}
		...
		if ($db->query($sql) === TRUE) {
			...
			$row_array['status'] =  "User registriert";
